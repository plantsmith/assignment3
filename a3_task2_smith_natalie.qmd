...
The dataset contains growth data with the following variables:
...
# save_kable(beta_table, "plots/beta_function_parameters.html")
...
To find some potential starting parameter values for our NLS analysis, we created an exploratory similar to Fig. 4 (biomass over time).
...
Below is a "good looking table" showing the selected parameter values as well as a graph:
...
The table below displays the lowest RMSE and the best-fitted models for each species:
...
low_rmse_M <- broom::tidy(low_rmse$nls_model[[1]]) %>% mutate(crop="Maize (M)")
low_rmse_S <- broom::tidy(low_rmse$nls_model[[2]]) %>% mutate(crop="Sweet sorghum (S)")
low_rmse_F <- broom::tidy(low_rmse$nls_model[[3]]) %>% mutate(crop="Fiber sorghum (F)")

# Combine coefficient data for all crops
...
# # Save kable
# save_kable(low_rmse_kable, "plots/low_rmse_kable.html")
```
...
hi_plot <- un_df %>%
  ...
  geom_line(aes(x = doy, y = smooth, linetype = crop)) +labs(y = " ", x = " ", color = " ")+
  theme_minimal()
...
```{r}
#| label: fig-two-crop
#| fig-cap: "Combined plot illustrating the relationship between fertilizer inputs and yields for all three grains, demonstrating higher yields with increased fertilizer inputs. The left graph represents high fertilizer inputs, while the right graph depicts low fertilizer inputs. "
